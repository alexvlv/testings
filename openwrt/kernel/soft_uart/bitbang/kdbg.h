...
#define TRACE_LVL(fmt, lvl, args...) \
	do { if(DBG_LVL>=(lvl)) printk ( \
		KERN_DEBUG "-T- " DEVICE " %s:" xstr(__LINE__) ":(%s) " fmt "\n", \
		kbasename(__FILE__),__PRETTY_FUNCTION__,  ## args); } while (0)

#define TRACE(fmt, args...) TRACE_LVL(fmt, TRACE_LVL_MIN, ## args)

#define VERB(fmt, args...) \
	do { if(DBG_LVL>1) printk ( \
		KERN_DEBUG "-V- " DEVICE " " fmt "\n",  ## args); } while (0)
#define DBG(fmt, args...) \
