                                                                                                                                   /*
 * udp_xor_proxy_multi_cleanup.c
 *
 * Multi-client bidirectional UDP XOR proxy with per-client upstream socket
 * and automatic idle client cleanup.
 *
 * Build:
 *   gcc -O2 -Wall -pthread -o udp_xor_proxy_multi_cleanup udp_xor_proxy_multi_cleanup.c
 */
...
// Get existing client or create new
struct client *get_client(struct proxy *p, struct sockaddr_in *client_addr) {
    ...
    if (!c) { pthread_mutex_unlock(&p->clients_lock); return NULL; }
    memcpy(&c->addr, client_addr, sizeof(c->addr));
    c->remote_fd = socket(AF_INET, SOCK_DGRAM, 0);
    if (c->remote_fd < 0) { perror("socket"); free(c); pthread_mutex_unlock(&p->clients_lock); return NULL; }
    c->last_seen = time(NULL);
    c->next = p->clients;
    p->clients = c;
    pthread_mutex_unlock(&p->clients_lock);
...
// Cleanup idle clients thread
void *cleanup_thread(void *arg) {
    struct proxy *p = (struct proxy *)arg;
    while (1) {
        sleep(CLEANUP_INTERVAL);
        time_t now = time(NULL);

        pthread_mutex_lock(&p->clients_lock);
        struct client **ptr = &p->clients;
        while (*ptr) {
            struct client *c = *ptr;
            if (now - c->last_seen > CLIENT_TIMEOUT) {
                printf("Removing idle client %s:%d\n",
...
int main(int argc, char *argv[]) {
    ...
    if (!remote_arg) { fprintf(stderr, "Remote address required (-r PORT or HOST:PORT)\n"); return 1; }
    ...
    pthread_create(&cleanup_tid, NULL, cleanup_thread, &p);
