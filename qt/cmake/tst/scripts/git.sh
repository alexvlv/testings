...
try_git || REV="Unknown"
echo "GIT Rev. $REV"
echo "#pragma once" > .git.h
echo "#define VERSION \"$REV\"" >> .git.h
echo "#define GIT_BRANCH \"$BRANCH\"" >> .git.h
echo "#define GIT_HASH \"$HASH\"" >> .git.h
echo "#define GIT_REV \"r$CNT-$HASH$STAR\"" >> .git.h
echo "#define GIT_DATE \"$DATE\"" >> .git.h
